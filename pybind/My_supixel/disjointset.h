...
public:
    ...
    /**
      ...
      */
     void ToClusters(vector<vector<int> >* clusters) const {
         assert(clusters);
         clusters->clear();

         int n_clusters = 0;
         vector<int> map(size_, -1);
         for (int i = 0; i < size_; ++i) {
             int p = Find(i);
    ...
    vector<int> numbers_;         // The size of disjoint subsets.
    mutable vector<int> subsets_; // The subset id of each element.
